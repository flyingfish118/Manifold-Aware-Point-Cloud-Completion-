...
def build_warmup_lambda_sche(opti, config):
    warmup_epochs = config.get('warmup_epochs', 5)
    decay_step = config.get('decay_step')
    lr_decay = config.get('lr_decay')
    lowest_decay = config.get('lowest_decay', 0.01)

    if decay_step is None or lr_decay is None:
        raise NotImplementedError("Missing decay_step or lr_decay in warmup config")

    def lr_lbmd(e):
        if e < warmup_epochs:
            return (e + 1) / warmup_epochs  # linear warmup
        else:
            decay = lr_decay ** ((e - warmup_epochs) / decay_step)
            return max(decay, lowest_decay)

    return torch.optim.lr_scheduler.LambdaLR(opti, lr_lambda=lr_lbmd)
