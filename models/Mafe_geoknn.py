...
from models.pointnet_Mafe_geoknn import  MLP_Res, UpLayer, fps_subsample, Transformer, DGCNN_Grouper, DGCNN_Grouper_geo, Transformer_geo
...
class UNet(nn.Module):
    def __init__(self, dim_feat=384, num_seeds = 1024, seed_fea=128):
        ...
        self.sa_module_1 = DGCNN_Grouper_geo(num_seeds//2, 16,[3, 8, seed_fea])
        self.pos_embed_1 = nn.Sequential(
            nn.Linear(3, 128),
            nn.GELU(),
            nn.Linear(128, seed_fea)
        )  
        self.transformer_1 = Transformer_geo(seed_fea, dim=64)
        self.sa_module_2 = DGCNN_Grouper_geo(num_seeds//8, 16,[seed_fea, seed_fea, 256])
        self.pos_embed_2 = nn.Sequential(
            nn.Linear(3, 128),
            nn.GELU(),
            nn.Linear(128, 256)
        )
        self.transformer_2 = Transformer_geo(256, dim=64)
        self.sa_module_4 = DGCNN_Grouper(1, num_seeds//16,[256, 256, dim_feat], ex_g=True)
    ...
    def forward(self, point_cloud):
        ...
        map_idx, nearest_dist = map_to_nearest_anchor(point_cloud, P_anchor.permute(0, 2, 1)) #b n 
        anchor_dist = isomap(P_anchor.permute(0, 2, 1)) #b n n

        ## Encoder        
        l1_xyz, l1_fea, fps_idx = self.sa_module_1(l0_xyz, l0_fea, P_anchor, map_idx, nearest_dist, anchor_dist)  # (B, 3, 512), (B, 128, 512)
        ...
        nearest_dist = torch.gather(nearest_dist, 1, fps_idx.long())  # (B, n)
        l1_fea = self.transformer_1(l1_fea, l1_xyz, fps_idx ,P_anchor, map_idx, nearest_dist, anchor_dist)

        l2_xyz, l2_fea, fps_idx = self.sa_module_2(l1_xyz, l1_fea, P_anchor, map_idx, nearest_dist, anchor_dist)  # (B, 3, 128), (B, 256, 128)
        map_idx  = torch.gather(map_idx, 1, fps_idx.long())  # (B, n)
        anchor_emb = build_anchor_geodesic_encoding(map_idx, anchor_dist)  # (N, 128)
        anchor_emb = self.mlp_proj_2(anchor_emb)
        l2_fea = l2_fea + anchor_emb.permute(0, 2, 1)
        nearest_dist = torch.gather(nearest_dist, 1, fps_idx.long())  # (B, n)
        l2_fea = self.transformer_2(l2_fea, l2_xyz, fps_idx, P_anchor, map_idx, nearest_dist, anchor_dist)
...
def map_to_nearest_anchor(P_all, P_anchor):
    """
    给定原始点云和代表点云，计算每个原始点最近的代表点索引和距离。
    ...
    Returns:
        ...
        nearest_dist: (B, N) 每个原始点到最近代表点的距离
    """
    B, _, N = P_all.shape
    ...
    # 取沿 M 维度的 argmin，得到最近代表点索引和距离
    nearest_dist, map_idx = torch.min(dists, dim=2)  # (B, N)

    return map_idx, nearest_dist
...
class Mafe_geoknn(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        """
        super(Mafe_geoknn, self).__init__()
