...
class DGCNN_Grouper_geo(nn.Module):
    ...
    def get_graph_feature(self, coor_q, x_q, coor_k, x_k, P_anchor, map_idx, nearest_dist, anchor_dist):
        ...
        with torch.no_grad():
            ...
            idx = knn_point_manifold(k*2, k, coor_q.transpose(-1, -2).contiguous(), P_anchor, map_idx, nearest_dist, anchor_dist) # B G M
    ...
    def forward(self, x, f, P_anchor, map_idx, nearest_dist, anchor_dist):
        '''
            INPUT:
                x : bs 3, N
        ...
        f = self.get_graph_feature(coor, f, coor, f, P_anchor, map_idx, nearest_dist, anchor_dist)
        f = self.layer1(f)
...
def knn_point_manifold(k_eucl, k_final, xyz, anchor_xyz, map_idx, nearest_dist, anchor_geo):
    """
    Args:
        k_eucl: int, 欧氏距离邻居数 (初选32个)
        k_final: int, 流形距离邻居数 (最终16个)
        xyz: [B, N, 3], 输入点云
        anchor_xyz: [B, M, 3], 锚点坐标
        map_idx: [B, N], 每个点对应的最近锚点索引
        nearest_dist: [B, N], 每个点到最近锚点的距离
        anchor_geo: [B, M, M], 锚点之间的流形距离矩阵

    Returns:
        group_idx: [B, N, k_final], 最终选择的邻居索引
    """
    B, N, _ = xyz.shape

    # Step 1: 初步欧氏距离knn，选取k_eucl个邻居
    sqrdists = square_distance(xyz, xyz)
    _, initial_group_idx = torch.topk(sqrdists, k_eucl, dim=-1, largest=False, sorted=False)  # [B, N, k_eucl]

    # Step 2: 计算流形距离（anchor-based）
    A_idx = map_idx  # [B, N]
    B_idx = torch.gather(map_idx.unsqueeze(-1).expand(-1, -1, k_eucl), 1, initial_group_idx)  # [B, N, k_eucl]

    # 点到各自anchor的欧氏距离
    dist_to_A = nearest_dist.unsqueeze(-1)  # [B, N, 1]
    dist_to_B = torch.gather(nearest_dist.unsqueeze(-1).expand(-1, -1, k_eucl), 1, initial_group_idx)  # [B, N, k_eucl]

    # anchor到anchor的流形距离
    anchor_to_anchor_dist = _gather_anchor_dist(anchor_geo, A_idx, B_idx)  # [B, N, k_eucl]

    # 组合总流形距离
    total_manifold_dist = dist_to_A + anchor_to_anchor_dist + dist_to_B  # [B, N, k_eucl]

    # 按流形距离重新排序取前k_final个邻居
    _, manifold_group_idx_sorted = torch.topk(total_manifold_dist, k_final, dim=-1, largest=False, sorted=False)  # [B, N, k_final]

    # 根据排序的索引提取原始点索引
    batch_indices = torch.arange(B, device=xyz.device).view(B, 1, 1).expand(-1, N, k_final)
    group_idx = initial_group_idx[batch_indices, torch.arange(N).view(1, N, 1), manifold_group_idx_sorted]
...
class Transformer_geo(nn.Module):
    def __init__(self, in_channel, dim=256, n_knn=16, pos_hidden_dim=64, attn_hidden_multiplier=4):
        super(Transformer_geo, self).__init__()
        self.n_knn = n_knn
        self.conv_key = nn.Conv1d(dim, dim, 1)
        self.conv_query = nn.Conv1d(dim, dim, 1)
        self.conv_value = nn.Conv1d(dim, dim, 1)
    ...
    def forward(self, x, pos, fps_idx, P_anchor, map_idx, nearest_dist, anchor_dist):
        """feed forward of transformer
        Args:
            x: Tensor of features, (B, in_channel, n)
            pos: Tensor of positions, (B, 3, n)
        ...
        pos_flipped = pos.permute(0, 2, 1).contiguous()
        idx_knn = knn_point_manifold(self.n_knn*2, self.n_knn, pos_flipped, P_anchor, map_idx, nearest_dist, anchor_dist).int() # B G M
        key = self.conv_key(x)
        value = self.conv_value(x)
        query = self.conv_query(x)
